...
func (mh *MyHandler) ListCards(w http.ResponseWriter, r *http.Request) {
	cookie, _ := r.Cookie("Cookie")
	userId := mh.GetCookie().GetUserId(cookie.Value)

	items, err := mh.GetDB().ListCards(userId)
	if err != nil {
		http.Error(w, "Something bad with ListCards", http.StatusInternalServerError)
		return
	}
